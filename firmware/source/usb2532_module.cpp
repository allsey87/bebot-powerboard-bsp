...
void CUSB2532Module::Write(uint16_t un_offset, uint8_t un_count, uint8_t* pun_buffer) {
   CTWController::GetInstance().BeginTransmission(HUB_CFGMODE_ADDR);
   CTWController::GetInstance().Write(un_offset >> 8);
   CTWController::GetInstance().Write(un_offset & 0xFF);
   CTWController::GetInstance().Write(un_count);
   for(uint8_t unIdx = 0; unIdx < un_count; unIdx++) {
      CTWController::GetInstance().Write(pun_buffer[unIdx]);
   }
...
void CUSB2532Module::Read(uint16_t un_offset, uint8_t un_count, uint8_t* pun_buffer) {
   ...
   CTWController::GetInstance().Read(HUB_CFGMODE_ADDR, un_count, true);
   for(uint8_t unIdx = 0; unIdx < un_count; unIdx++) {
      pun_buffer[unIdx] = CTWController::GetInstance().Read();
   }
}
